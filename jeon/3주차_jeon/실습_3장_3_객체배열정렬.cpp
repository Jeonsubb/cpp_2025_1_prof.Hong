...
//클래스 멤버 함수인걸 표시하기 위해서 클래스명:: 으로 어떤 namespace에 속하는지 명시,,, std:: 이런거
...
int Rectangles::compareRectangless(const Rectangles* b) {
    ...
    //x 같은지 비교 -> y 같은지 비교 -> width 같은지 비교 -> height 같은지 비교
    if (LBXCompare != 0)
        return LBXCompare;
...
void showRectangless(string message, Rectangles* rectangles[], int numRectangless) {
    ...
    for (int i = 0; i < numRectangless; ++i) {
        rectangles[i]->showRectangles(); //마찬가지로 멤버 함수를 부를 때 포인터 클래스 객체가 함수 멤버함수 호출할 때는 -> 이렇게
    }
    cout << endl;
}
